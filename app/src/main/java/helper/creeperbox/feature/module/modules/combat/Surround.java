...
@ModuleInfo(name = "环绕", category = Category.Combat)
public class Surround extends Module {
    ...
    private final NumberValue speed = new NumberValue("速度", this,20,1, 50, 1);
    ...
    public void onTick(TickEvent event){
        ...
        if(target!=null){
            angle+=speed.getCurrentValue().floatValue();
            angle = MathUtil.wrapAngleTo180_float(angle);
            Vec3f pos = target.getPos();
            float direction = (float) Math.toRadians(angle+180);
